...
export function ApiKeys() {
    ...
    const toggleMutation = useMutation({
        ...
        onSuccess: (_data, variables) => {
            // Optimistically update cache so toggle/status change immediately
            queryClient.setQueryData<{ apiKeys: typeof apiKeys } | undefined>(
                ["api-keys"],
                (prev) => {
                    if (!prev) return prev;
                    return {
                        apiKeys: prev.apiKeys.map((k) =>
                            k.id === variables.id ? { ...k, disabled: variables.disabled } : k,
                        ),
                    };
                },
            );
        ...
        <DashboardLayout>
            <div className="space-y-8">
                ...
                <Card className="bg-zinc-950/70 border border-zinc-800/80">
                    <CardHeader>
                        <CardTitle className="text-lg text-zinc-50">Create new key</CardTitle>
                        ...
                        >
                            <div className="flex-1">
                                ...
                                <Input
                                    ...
                                    className="h-10 bg-zinc-900/95 border border-zinc-400 text-zinc-100 placeholder:text-zinc-500"
                                    required
                                />
                            </div>
                            <Button
                ...
                <div>
                    <h2 className="text-sm font-semibold mb-4 text-zinc-200">
                        ...
                        {!apiKeysQuery.isLoading && (
                            <span className="font-normal ml-2 text-zinc-400">
                                ({apiKeys.length})
                            </span>
                        )}
                    </h2>
                    ...
                    {apiKeysQuery.isLoading ? (
                        ...
                        <Card className="bg-zinc-950/60 border border-zinc-800/80 border-dashed">
                            <CardContent className="pt-6">
                                <div className="text-center py-8">
                                    <Key className="size-10 text-zinc-600 mx-auto mb-3" />
                                    <p className="text-sm text-zinc-300">No API keys yet</p>
                        ...
                        <div className="rounded-xl border border-zinc-800/80 bg-zinc-950/70 overflow-hidden">
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="border-b border-border/50">
                                        <th className="text-left px-4 py-3 text-xs font-medium text-zinc-400">Name</th>
                                        <th className="text-left px-4 py-3 text-xs font-medium text-zinc-400">Key</th>
                                        <th className="text-left px-4 py-3 text-xs font-medium text-zinc-400">Status</th>
                                        <th className="text-right px-4 py-3 text-xs font-medium text-zinc-400">Credits Used</th>
